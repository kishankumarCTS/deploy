...
import PoolStepper from "@/components/Modals/createPool/Stepper";
...
function PoolsTab() {
  ...
  const rows = ROWS.map((row: any) => ({
    ...
    tlsEnabled: <DataBadge data={row.tlsEnabled} />,
    createdAt: (
      <div className="flex items-center gap-2">
        <FaRegCalendar size={18} /> {row.createdAt}
      </div>
    ),
  }));
  ...
  // Handler for successful pool creation
  const handlePoolCreated = () => {
    // Refresh the table data here
    // You might want to call an API to refetch pools
    console.log("Pool created successfully, refreshing data...");
  };
      ...
      <div>
        ...
        <div className="py-6 px-3 bg-[#EEF5FE] rounded-[20px]">
          ...
          <Table
            ...
            mainAction={{
              children: "View Pool Details",
              onClick: (poolId: string) => handlePoolDetails(poolId),
            }}
