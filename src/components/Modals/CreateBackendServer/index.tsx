...
interface BackendServerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit?: (servers: BackendServer[]) => void;
}

function BackendServerModal({
  isOpen,
  onClose,
  onSubmit,
}: BackendServerModalProps) {
    ...
    },
    {
      ...
      cell: (row, rowIndex) => (
        <InputField
          ...
          onChange={(e) =>
            handleEdit(rowIndex, "monitorAddress", e.target.value)
          }
        />
      ),
    },
          ...
          <>
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 my-4 flex items-center">
              ...
              <p className="text-sm text-yellow-700">
                Ensure the backend server has an active service running on the
                monitor port.
              </p>
            </div>
...
export default BackendServerModal;
